...
import { StoreTODO } from "../redux/features /todoSlice";
import { setTodos } from "../utils/setTodos";
import { useCallback, useEffect, useState } from "react";
import type { AppStore } from "../redux/store";

const AddTask = ({ visible, onClose }: { visible: boolean; onClose: () => void; }) => {
  ...
  const onAddTask = useCallback(() => {
    ...
    if (!nameError && !descError) {
      const tempTodo =  setTodos(title, description, priority, category, dueDate);
      dispatch(StoreTODO({tempTodo, uId}));
      // Reset form
      setTitle("");
      setDescription("");
  ...
  }, [nameError, descError, title, description, priority, category, dueDate, onClose, dispatch,uId  ]);
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
              <div>
                <label htmlFor="a-priority" className="block text-sm font-medium text-gray-700 mb-1">
                  Priority
                </label>
                <select
                  name="a-priority"
