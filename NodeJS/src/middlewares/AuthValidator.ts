...
export const registerValidator = [
  body('name')
    .notEmpty().withMessage('Name must have more than 5 characters')
    .isLength({ min: 5, max: 50 }).withMessage('Name must have max 50 and min 5 characters'),

  body('username')
    .notEmpty().withMessage('Username must have more than 5 characters')
  ...
  body('password')
    ...
    .matches(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z]{8,20}$/).withMessage('Password should be combination of one uppercase , one lower case, one digit and min 8 , max 20 char long')
];
...
export const loginValidator = [
  ...
  body('password')
    .notEmpty().withMessage('email must have more than 8 characters')
    .isLength({ min: 8, max: 20 }).withMessage('email must hav min 8 or max 20 characters')
    .matches(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z]{8,}$/).withMessage('Password should be combination of one uppercase , one lower case, one digit and min 8 , max 20 char long')
